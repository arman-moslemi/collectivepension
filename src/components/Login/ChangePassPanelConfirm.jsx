...
const ChangePasswordPanelConfirm = () => {
    ...
    const validationSchema = Yup.object().shape({
        ...
        oldPassword: Yup.string()
            .required('رمز عبور قدیمی الزامی است').min(8, 'رمزعبور باید حداقل 8 کاراکتر باشد'),

    });
    const [captchaWord, setCaptchaWord] = useState("");
    const [reCap, setRecap] = useState(false);
    const [csfr, setCsfr] = useState();
    ...
    const getCsfr = async () => {
        try {
            ...
            const response = await axios.get(apiUrl + "Auth/csrf",{
                  headers: {
    ...
    const handleSubmit = async (values, { setSubmitting }) => {
        ...
        try {
            ...
            const csrfToken = await getCsfr();


            var updateOrg = await axios.post(
                apiUrl + "Auth/OldPassword",
                {
                ...
                },
                {
                    ...
                    headers: {
                        Authorization: `Bearer ${cookies.get('token')}`, // ✅ حتماً با backtick
...
export default ChangePasswordPanelConfirm;