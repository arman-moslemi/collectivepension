...
const ExpertDefinition = () => {
    ...
    const [thereIsExpert,
        setThereIsExpert] = useState(false);
    const [expertDefinitionModal,
        setExpertDefinitionModal] = useState(false);
    let navigate = useNavigate();

    return (
        <div
            className="w-full flex flex-col items-center rounded-[6px] bg-white px-[25px] py-[17px]">
            {thereIsExpert
          ...
          <BarChart
              ...
              >
                ...
                : <div className="w-full pt-12 pb-16">
                    <MainPicText
                        ...
                        text={<div> <p className="text-mainBlue text-[20px] md:text-[12px] font-IRANYekanBold">شما تاکنون کارشناسی تعریف نکرده‌اید !</p> </div>}
                        pageButton={<div className = "flex" > <div className="ml-3 w-[186px] md:w-[150px]"><MainButton
                        onClickFunction={() => setExpertDefinitionModal(true)}
                        label={'تعریف کارشناس'}/></div> </div>}/>
                </div>
}
            ...
            {expertDefinitionModal
                ...
                big={false}
                        ...
                        text={<div className = "w-full grid grid-cols-2 gap-4" > <div>
                        <MainInput label={'نام'} holder={'نام را بنویسید'}/>
                    </div> <div > <MainInput label={'نام خانوادگی'} holder={'نام خانوادگی را بنویسید'}/> </div> <div> <MainInput label={'کدملی'} holder={'کدملی را بنویسید'}/> </div> < div > <MainInput label={'شماره تماس'} holder={'شماره تماس را بنوسید'}/> </div> <div> <MainInput label={'نام کاربری'} holder={'نام کاربری را بنویسید'}/> </div> < div > <MainInput label={'رمزعبور'} holder={'رمز عبور را بنویسید'}/> </div> </div>}
                        modalButton={< div className = "w-full flex justify-center" > <div className="w-[134px]"><MainButton
                        onClickFunction={() => {
                        setThereIsExpert(true);
                        setExpertDefinitionModal(false)
                    }}
                        label={'ثبت کارشناس'}/></div> </div>}/>
                : null}
