const MainExplanation = ({color,text}) => {
    return (
        <div className={`w-full ${color === 'red' ? 'bg-backRed' : color === 'green' ? 'bg-backGreen'  : color==='yellow'? 'bg-backYellow':'bg-backBlue' } rounded-[6px] flex`}>
            ...
            <p className={`py-[16px] ${color === 'red' ? 'text-[12px] text-mainRed' : color === 'green' ? 'text-[12px] text-mainGreen' :  color==='yellow' ? 'text-[15px] text-darkYellow' : 'text-[15px] text-buttonBlue' } pr-[8px] pl-[42px] font-IRANYekanBold text-justify  leading-6`}>{text}</p>
